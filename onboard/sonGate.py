...
# subs_socks.append(zmq_wrapper.subscribe([zmq_topics.topic_stereo_camera],zmq_topics.topic_camera_port, '172.17.0.2'))
keep_running=True
...
if args.localSend:
    udpImIpPort = ('', int(config.sonar_udpPort))
else:
    udpImIpPort = (config.groundIp, int(config.sonar_udpPort))
...
async def recv_and_process():
    ...
    while keep_running:
        ...
        for sock in socks:
            ...
            if ret[0]==zmq_topics.topic_sonar:
                inImgCnt += 1
