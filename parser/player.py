    ...
    def vidProc(self, curTopic, im, imLowRes, timestamp):
        ...
        if im is not None:
            ...
            if self.saveTiff and False: # ignore HQ tiff
                ...
                path = os.path.join(self.imgsPath, curImName + "_HQ" + ".tiff")
                print(path)
                cv2.imwrite( path, im )
                # print(im[:20])
...
if __name__=='__main__':
    ...
    # print('self.saveAvi', self.saveAvi)
    # print('self.highSpeed', self.highSpeed)
    player = Player(recPath=args.recPath, skipFrame=args.skipFrame, saveTiff=args.saveTiff, showVideo=args.showVideo, freeRun=args.freeRun, highQuality=args.highQuality, saveAvi=args.saveAvi, generateDataFrame=False)
    player.parse()
